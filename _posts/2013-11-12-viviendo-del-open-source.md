---
layout: post
title: Viviendo del Open Source
description: "Hablamos con Alejandro Revilla de JPos, su historia desde empezar a programar hasta vivir de una empresa desarrollada a partir de un producto open source."
modified: 2013-11-12
category: articles
tags:
comments: true

podcast: true
episode_number: "005"
episode_name: Entrevista_a_Alejandro_Revilla
duration: 1:26:31
length: 84377061

---

{% capture episode_path %}{{site.s3}}/{{page.episode_number}}/{{page.episode_number}}-{{page.episode_name}}{% endcapture %}

{{ page.description }}

## Audio
<audio controls>
  <source src="{{ episode_path }}.mp3" type="audio/mpeg">
  <source src="{{ episode_path }}.ogg" type="audio/ogg">
  Si ves esto tu navegador no soporta la reproducción en HTML5 de archivos mp3 ni ogg, vas a tener que bajar el archivo. :(
</audio>

### iTunes
* [Surculus Fructum en iTunes](itmss://itunes.apple.com/podcast/presentacion-docker-y-dokku/id709608207)


### Descargas

* [episodio en mp3]({{ episode_path }}.mp3)
* [episodio en m4a]({{ episode_path }}.m4a)
* [episodio en ogg]({{ episode_path }}.ogg)


# Links
* [Offline.js](http://github.hubspot.com/offline/docs/welcome)
* [Ink](http://zurb.com/ink/)
* [Sale Python 3.4](http://docs.python.org/dev/whatsnew/3.4.html)
* - [pip](http://www.pip-installer.org/en/latest/)
* - [asyncio](https://pypi.python.org/pypi/asyncio/0.1.1)
* [Docker en la Raspberri Pi](http://resin.io/docker-on-raspberry-pi/)
* [Engine yard no va a seguir sponsoreando rubinius](http://rubini.us/2013/10/11/the-once-and-future-rubinius)
* [nsq por Bit.ly](https://github.com/bitly/nsq)
* [Garbage Collector Incremental en PyPy](http://morepypy.blogspot.com/2013/10/incremental-garbage-collector-in-pypy.html)
* [Pull request con rediseño de ruby-lang.org](https://github.com/ruby/www.ruby-lang.org/pull/342)
* [CircleCI cierra temporalmente por falla de seguridad en MongoHQ](http://www.zdnet.com/circleci-temporarily-shuts-doors-amid-mongohq-hack-7000022652/)
* [Minar Bitcoins desde el navegador de tus usuarios](http://www.tidbit.co.in)


## Entrevista: Alejandro Revilla

Alejandro es el creador de JPOS, un producto open source que es la impelmentación abierta de software para manejo de transacciónes de tarjetas de credito
más usada en el mundo.

## Links

* [@apr](https://twitter.com/apr) en twitter.
* [ar en github](https://github.com/ar)
* [POS](http://es.wikipedia.org/wiki/Terminal_punto_de_venta)
* [jPOS](http://jpos.org)
* [BBS](http://es.wikipedia.org/wiki/Bulletin_Board_System)
* [AX.25](http://en.wikipedia.org/wiki/AX.25)
* [ISO 8583](http://es.wikipedia.org/wiki/ISO_8583)
* [AWT](http://es.wikipedia.org/wiki/Abstract_Window_Toolkit)
* [OSGi](http://en.wikipedia.org/wiki/OSGi)

## Conferencias y Eventos

* [Grupo de meetups de js en Buenos Aires](http://www.meetup.com/Meetup-js/)
* [Tech Meetup Uruguay](http://tech.meetup.uy/)
* [RubyConf Argentina](http://rubyconfargentina.org/es/)


## Guarangada de la semana

* Ylvis:
* - [Jan Egeland](http://www.youtube.com/watch?v=FUyOV3a-wtg)
* - [Stonehenge](http://www.youtube.com/watch?v=Q2cOQLv6DCk)
* - [The Fox](http://www.youtube.com/watch?v=H0T3x8OScws)
